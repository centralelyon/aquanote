...
export function createImageControlPoints(config = {}) {
  ...
  const radius = finiteOr(config.radius, 7);
  ...
  const drawMode = config.drawMode ?? config.mode;
  ...
  canvas.addEventListener("pointerdown", (event) => {
    ...
    if (hitIndex >= 0) {
      ...
      state.interaction = { type: "point", index: hitIndex };
    } else if (zoomEnabled) {
      state.interaction = {
        type: "pan",
        x: local.x,
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    if (state.interaction.type === "point") {
      ...
      state.points[state.interaction.index] = clampToImage
  ...
  canvas.addEventListener("dblclick", (event) => {
    if (!addPointOnDoubleClick || state.points.length >= maxPoints) {
      return;
    }
